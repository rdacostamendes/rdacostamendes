# Olá, eu sou o Rodrigo! 👋

Sou estudante do 1º semestre de **Análise e Desenvolvimento de Sistemas (ADS)**, focado em construir uma base sólida em engenharia e desenvolvimento de software.

### 🛠️ No que estou focado atualmente:
* **Linguagens:** Aprendizado contínuo em **Python** para automações e lógica, e **C** focado em estrutura de dados, ponteiros e gerenciamento de memória.
* **Infraestrutura:** Experiência prática em montagem, manutenção de computadores/notebooks e cabeamento de redes.

### 📌 Projetos em Destaque:
* [GestaoDeAtivosNaTecnoLogSA](https://github.com/rdacostamendes/GestaoadeativosnaTecnoLogSA): Módulo ERP em C utilizando alocação dinâmica de memória (`realloc`) e aritmética de ponteiros para fechamento de balanço patrimonial.

📫 Como me encontrar: [Seu LinkedIn aqui]
